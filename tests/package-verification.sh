...
mkdir -p "${fixture_directory}/uk-cookie-consent-manager/docs"
printf '%s\n' 'Configure UCCM_MANIFEST_PRIVATE_KEY in GitHub Actions.' > "${fixture_directory}/uk-cookie-consent-manager/docs/OPERATING-GUIDE.md"
(
  cd "${fixture_directory}"
  zip -qr documented-secret-name.zip uk-cookie-consent-manager
)
...
printf '%s\n' "${fake_signing_key}" > "${fixture_directory}/uk-cookie-consent-manager/leaked-secret.txt"
(
  cd "${fixture_directory}"
  zip -qr leaked-secret.zip uk-cookie-consent-manager
)
...
printf '%s\n' 'ghp_AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA' > "${fixture_directory}/uk-cookie-consent-manager/leaked-secret.txt"
(
  cd "${fixture_directory}"
  zip -qr leaked-token.zip uk-cookie-consent-manager
)

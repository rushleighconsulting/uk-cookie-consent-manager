...
Rushleigh Cookie Choices 1.0.2 is the reviewed stable release following
controlled release-candidate and WordPress.org submission-package acceptance.
...
= 1.0.2 =
* Renames the plugin and WordPress.org identity to Rushleigh Cookie Choices with the distinctive `rushleigh-cookie-choices` slug.
* Documents the reviewer-requested nonce, capability, sanitisation, licensing and functionality audit.
* Clarifies that the inert `type="text/plain"` resource marker is consent metadata and cannot be enqueued without defeating prior blocking.

= 1.0.1 =
* Removes obsolete manual text-domain loading so modern WordPress can use just-in-time language packs without the discouraged call.
* Records the manual security and performance disposition of the remaining WordPress Plugin Check warnings.
* Preserves the accepted 1.0.0 consent, privacy, scanning, accessibility and Multisite behaviour.

= 1.0.0 =
* First stable release following controlled WordPress Multisite and Windows 11/NVDA acceptance.
* Provides accessible consent choices, granular preferences, withdrawal and persistent Cookie settings.
* Includes prior blocking, reviewed inventory and scanning, privacy-preserving consent evidence, translation support and Multisite operation.
* Keeps GitHub release provenance separate from the native WordPress.org distribution and update channel.
